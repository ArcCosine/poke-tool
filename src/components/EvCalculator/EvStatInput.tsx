...
export const EvStatInput: React.FC<EvStatInputProps> = ({
  ...
  return (
    <div className="flex items-center gap-1 sm:gap-1.5 bg-slate-100/70 dark:bg-slate-900/60 p-1 rounded-xl border border-slate-200/80 dark:border-slate-800/80 w-fit">
      {/* 0 (最小値) 専用ボタン */}
      <button
        type="button"
        onClick={handleReset}
      ...
      <input
        ...
        disabled={disabled}
        className="h-8 w-12 text-center text-sm font-black bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-slate-100 shadow-inner box-border"
      />
