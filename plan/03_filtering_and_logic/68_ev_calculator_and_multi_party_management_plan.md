...
1.  **努力値計算ツール (`EvCalculator.tsx`) の新設**:
    *   ポケモン、性格（セレクトボックス）、努力値（各ステータス 0〜32、合計 66 制限）、特性、持ち物、技（最大4つ）を設定可能。
    *   努力値入力によって、リアルタイムで実数値（レベル50）を計算・表示。
...
*   **修正後の実装**:
    ...
    ```
*   **TDD の実施**:
    `src/utils/party.test.ts` に、努力値および性格補正を設定した際の実数値が、ゲーム内の正しい数値（例: ハッサムA特化で198など）と一致することを検証するテストコードを最初に追加します。
...
*   努力値入力: 努力値上限（各32、合計66）をチェックする入力。
